...
async function main() {
    ...
    for (const service of servicesData) {
        try {
            ...
            console.log(`  ✅ ${newService.name} - ${newService.price.toLocaleString('vi-VN')} VND`);
        }
        catch (error) {
            if (error.code === 'P2002') {
    ...
    if (allStaff.length > 0 && createdServices.length > 0) {
        ...
        for (const service of createdServices) {
            const numStaff = Math.floor(Math.random() * 3) + 2;
            ...
            try {
                ...
                console.log(`  ✅ ${service.name}: ${selectedStaff.map((s) => s.name).join(', ')}`);
            }
            catch (error) {
    ...
    console.log(`📊 Total: ${allStaff.length} staff, ${createdServices.length} new services created`);
}
main()
    .catch((e) => {
...
//# sourceMappingURL=seed-duck.js.map